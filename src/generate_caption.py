...
# 2026-09-10、指示を増やした結果 stop_reason=max_tokens で JSON が途中で切れ、
# テンプレート文へ落ちる事象が出たため引き上げた。8000でも1回目が空になることが
# あったため、リトライに頼らず1回で収まる値にしている。
MAX_TOKENS = 16000
...
- threads: instagram より少しだけ砕けた程度。文末は基本ですます調のままで、
  「〜した。」「〜だよね」「〜かも」のような話し言葉の言い切りは使わない。
  短めのテンポにする一方、乱暴・軽薄な印象にならないようにする。
...
- 最初の一文で「誰の、どういう場面の話か」が分かるようにしてください。
  新卒／中途社員／管理職といった対象と、その人が置かれている状況を先に置きます。
  記事を読んでいない人が読むため、本文の途中から切り出した断片にしないでください。
- 「本人」「その担当者」のような指示語で始めないでください。最初に誰を指すのかを
  書いてから使ってください。
- 別の人に読める言い方を避けてください。たとえば「中途採用の担当者」は人事の
  採用担当者に読めます。中途入社した社員を指すなら「中途入社の社員」「中途で
  入った人」のように、誰のことか取り違えようのない言い方にします。
- 「思い浮かべてください」「想像してみてください」のような、読み手に指示する
  命令形で始めないでください。状況をそのまま書き始めます。
- 「〜が必要です」「〜すべきです」「〜しましょう」のような、読み手を諭す言い方で
  締めないでください。記事が示している見方や要点を、事実として書いて終えます。
- 一文は40字前後、長くても60字までにしてください。3〜4文に分けます。
  一つの文に条件と説明を詰め込まず、区切って順に書いてください。
- 短くするために語を削らないでください。削ると意味が通らなくなる語（「〜のまま」
  「〜だけ」が何と対になっているか、など）は必ず残します。字数に収まらないなら、
  文を増やすか、その要点自体を選び直してください。
- 原因と結果の間に段階があるなら、飛ばさずに書いてください。読み手が「なぜそう
  なるのか」を自分で補わないと意味が通らない文にしないでください。
- 「〜という〜を〜する」のように修飾を重ねないでください。読み手が一息で
  読めない文は、内容が正しくても届きません。
- 「記事は〜を挙げています」のような紹介文にしないでください。記事の要約では
  なく、その場で読んで意味が通る文章として書きます。
- 本文180字以内。記事本文にない事実を足さない点は上の指示と同じです。
- 本文が条件つきで述べていること（「AでないのにBなら」など）を、条件を落として
  断定に変えないでください。記事の判定基準が消えてしまいます。
- 誰が誰に対して何をするのかを、本文どおりに保ってください。主語と客体を
  入れ替えないでください。
- 字数に収まらない、または条件を保ったまま書けない要点は選ばないでください。
  無理に圧縮せず、別の要点に変えてください。
- 最後の一文には、記事が示している見方や答えを置いてください。問題の描写だけで
  終わらせないでください。「〜と評価されます」「〜が起きます」のように起きた事の
  説明で締めると、不満を並べただけの投稿になります。記事が「本当の原因は何か」
  「何を変えるべきか」を書いているなら、そこに触れて終わります。
- 時点や場面を修飾節でくくらないでください。「本部が方針を変えた実施日」のように
  書くと、方針を変えた日なのか実施する日なのかが読み手に判別できません。
  いつの話かは文を分けて書いてください。
"""
